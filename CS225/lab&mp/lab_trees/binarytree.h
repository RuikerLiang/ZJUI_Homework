...
private:
   ...
   */
  template <typename U> void _swap(U &a, U &b);
   ...
   * @param enable_min
   */
  bool isOrdered(const Node *subRoot, T max, T min, bool enable_max,
                 bool enable_min) const;
   ...
   * @param paths vector of vectors that contains path of nodes
   */
  void printPaths(Node *subRoot, vector<T> curr_path,
   ...
   * @param dep
   */
  int sumDistances(Node* subRoot, int dep) const;
